...
func generate_hints():
	for direction in [Vector2(-1,-1), Vector2(-1, 1), Vector2(1, -1), Vector2(1, 1)]:
		for l in range(1, info.action["range"]+1):
			level.hints.generate_hint(Color.RED, global_position + direction * level.info.TILE_SIZE * l)
...
func choose_action():
	if is_player_visible():
		...
		if displacement.x == displacement.y and displacement.x <= info.action["range"]:
